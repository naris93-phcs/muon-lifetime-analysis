...
OUTPUT_DIR = PROJECT_ROOT / "results" / "root_full_dataset"

OUTPUT_CSV = OUTPUT_DIR / "fit_range_stability.csv"

OUTPUT_PLOT = OUTPUT_DIR / "fit_range_stability.png"
...
def load_decay_times(csv_path: Path) -> np.ndarray:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"Accepted-candidate CSV was not found:\n{csv_path}")

    dataframe = pd.read_csv(csv_path)

    if "decay_time_us" not in dataframe.columns:
        raise ValueError("Column 'decay_time_us' was not found in the CSV.")
    ...
    decay_times = decay_times[np.isfinite(decay_times)]
...
def truncated_exponential_mean(
    ...
    return tau - fit_width_us / np.expm1(fit_width_us / tau)
...
def estimate_truncated_tau(
    ...
    sample_mean = float(np.mean(shifted_times_us))

    if sample_mean <= 0:
        raise ValueError("Shifted-time mean must be positive.")
    ...
    for _ in range(200):
        middle_tau = (lower_tau + upper_tau) / 2.0
    ...
    return (lower_tau + upper_tau) / 2.0
...
def bootstrap_tau_uncertainty(
    ...
    fit_width_us = fit_max_us - fit_min_us

    shifted_times = selected_times_us - fit_min_us
...
def analyze_fit_range(
    ...
    selected = decay_times_us[
        (decay_times_us >= fit_min_us) & (decay_times_us <= fit_max_us)
    ]
    ...
    shifted_times = selected - fit_min_us

    # This ignores the finite upper acquisition limit.
    simple_shifted_tau = float(np.mean(shifted_times))
...
def plot_stability(
    ...
    valid = results[np.isfinite(results["truncated_tau_us"])].copy()

    if valid.empty:
        print("No valid lifetime estimates were available for plotting.")
        return
    ...
    axis.set_xlabel("Lower fit boundary $t_{min}$ (μs)")
...
def main() -> None:
    ...
    decay_times_us = load_decay_times(INPUT_CSV)
    ...
    print_results(results_dataframe)

    print(f"\nSaved fit-range table:\n{OUTPUT_CSV}")

    plot_stability(results_dataframe)
