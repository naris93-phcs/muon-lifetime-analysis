...
def load_accepted_candidates(
    ...
    dataframe["event_index"] = (
        dataframe["event_index"].astype(int)
    )
    ...
    if dataframe.empty:
        raise ValueError(
            "No accepted candidates were found."
        )
...
def load_event(
    ...
    if len(arrays["time"]) == 0:
        raise IndexError(
            f"Event {event_index} was not found."
        )
...
def create_contact_sheets(
    ...
    with uproot.open(
        ...
        print(
            f"ROOT file: {root_file_path.name}"
        )
        print(
            f"ROOT events: {total_root_events}"
        )
        print(
            f"Accepted candidates: {len(accepted)}"
        )
        print(
            f"Contact-sheet pages: {number_of_pages}"
        )
        print()
